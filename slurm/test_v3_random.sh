#!/bin/bash
#SBATCH --job-name=v3_rand
#SBATCH --output=/gpfs/workdir/malhotraa/ConMax3D_reproduce/logs/v3r_%A_%a.out
#SBATCH --error=/gpfs/workdir/malhotraa/ConMax3D_reproduce/logs/v3r_%A_%a.err
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=04:00:00
#SBATCH --exclude=ruche-gpu18,ruche-gpu13
#SBATCH --export=NONE
# Random baseline for all 16 scenes
# Submit:
#   sbatch --partition=gpua100 --cpus-per-task=8 --array=0-15 slurm/test_v3_random.sh
...
SEED=${SEED:-42}
RESULTS=$WORKDIR/ConMax3D_reproduce/results/v3_random_s${SEED}
...
# Phase 1: Random selection (no GPU needed, just generates indices)
...
INDICES_FILE="$RESULTS/$SCENE/train_indices_${SCENE}_random.json"
...
echo "=== gsplat: $SCENE/random | Indices: $INDICES ==="

GSPLAT_RESULT=$RESULTS/$SCENE/gsplat_random
...
if os.path.isdir(stats_dir):
    for f in sorted(os.listdir(stats_dir)):
        if f.endswith('.json'):
            ...
            ssim = s.get('ssim', 0)
            lpips = s.get('lpips', 0)
            print(f'$SCENE/random: PSNR={psnr:.4f} SSIM={ssim:.4f} LPIPS={lpips:.4f}')
"
