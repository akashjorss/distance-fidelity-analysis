#!/bin/bash
#SBATCH --job-name=v2_kmeans
#SBATCH --output=/gpfs/workdir/malhotraa/ConMax3D_reproduce/logs/v2k_%A_%a.out
#SBATCH --error=/gpfs/workdir/malhotraa/ConMax3D_reproduce/logs/v2k_%A_%a.err
...
RESULTS=$WORKDIR/ConMax3D_reproduce/results/v2_kmeans
GSPLAT_TRAINER=$WORKDIR/gsplat/examples/simple_trainer.py
GSPLAT_PYTHON=$WORKDIR/conda_envs/gsplat_env/bin/python

# 12 configs: 4 scenes × 3 thresholds (5, 20, 40)
SCENES=(fern trex Church Museum fern trex Church Museum fern trex Church Museum)
DATA_BASES=($WORKDIR/data/LLFF $WORKDIR/data/LLFF $WORKDIR/data/Tanks $WORKDIR/data/Tanks \
            ...
            $WORKDIR/data/LLFF $WORKDIR/data/LLFF $WORKDIR/data/Tanks $WORKDIR/data/Tanks)
DATA_FACTORS=(4 4 1 1 4 4 1 1 4 4 1 1)
MAX_IMAGES=(0 0 150 0 0 0 150 0 0 0 150 0)
THRESHOLDS=(5 5 5 5 20 20 20 20 40 40 40 40)
...
echo "=== $TAG: K-Means + elbow, threshold=$THRESH ==="
...
# ── Phase 1: SAM2 + K-Means + strategies ──
