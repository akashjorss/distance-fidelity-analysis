#!/bin/bash
#SBATCH --job-name=gsplat_tt
#SBATCH --output=logs/gsplat_tt_%A_%a.out
#SBATCH --error=logs/gsplat_tt_%A_%a.err
...
DATA_DIR=$WORKDIR/data/Tanks/$SCENE
RESULTS_BASE=$WORKDIR/ConMax3D_reproduce/results
RESULT_DIR=$RESULTS_BASE/gsplat_tt/$FRAME_COUNT/$METHOD/$SCENE
INDICES_FILE=$RESULTS_BASE/tt/$FRAME_COUNT/$METHOD/train_indices.json
...
python simple_trainer.py default \
    ...
    --wandb_run_name "tt_${SCENE}_${METHOD}_${FRAME_COUNT}" \
    --disable_viewer
...
echo "3DGS training done for T&T: $SCENE $METHOD $FRAME_COUNT"
