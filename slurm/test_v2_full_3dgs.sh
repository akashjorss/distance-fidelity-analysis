#!/bin/bash
#SBATCH --job-name=v2_3dgs
#SBATCH --output=/gpfs/workdir/malhotraa/ConMax3D_reproduce/logs/v2full_%A_%a.out
#SBATCH --error=/gpfs/workdir/malhotraa/ConMax3D_reproduce/logs/v2full_%A_%a.err
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=06:00:00
#SBATCH --exclude=ruche-gpu18,ruche-gpu13
#SBATCH --export=NONE
# NOTE: Submit with partition + cpus + array overrides:
#   sbatch --partition=gpu    --cpus-per-task=10 --array=0-7  slurm/test_v2_full_3dgs.sh  # LLFF on V100
#   sbatch --partition=gpua100 --cpus-per-task=8 --array=8-15 slurm/test_v2_full_3dgs.sh  # T&T on A100
...
echo "=== [$IDX] $SCENE | 3DGS | submodular_geometric vs pure_fvs ==="
...
# ══════════════════════════════════════════
# Phase 1: SAM2 + concepts + frame selection
# ══════════════════════════════════════════
...
# ══════════════════════════════════════════
# Phase 2: gsplat training for each strategy
# ══════════════════════════════════════════
