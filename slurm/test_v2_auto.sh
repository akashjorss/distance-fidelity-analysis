#!/bin/bash
#SBATCH --job-name=v2_auto
#SBATCH --output=/gpfs/workdir/malhotraa/ConMax3D_reproduce/logs/v2auto_%A_%a.out
#SBATCH --error=/gpfs/workdir/malhotraa/ConMax3D_reproduce/logs/v2auto_%A_%a.err
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=06:00:00
#SBATCH --exclude=ruche-gpu18,ruche-gpu13
#SBATCH --export=NONE
# Auto-threshold test on representative scenes
# Submit:
#   sbatch --partition=gpu    --cpus-per-task=10 --array=0-7  slurm/test_v2_auto.sh  # LLFF
#   sbatch --partition=gpua100 --cpus-per-task=8 --array=8-15 slurm/test_v2_auto.sh  # T&T

set -ex

WORKDIR=/gpfs/workdir/malhotraa
SCRIPTS=$WORKDIR/ConMax3D_reproduce/scripts
RESULTS=$WORKDIR/ConMax3D_reproduce/results/v2_auto
...
K=10
# threshold=0 means auto-detect
THRESH=0
STRATEGIES="submodular_geometric,pure_fvs"

echo "=== [$IDX] $SCENE | AUTO threshold ==="
